...
function getStorageBucketCandidates(bucketName) {
  ...
  if (bucketName) {
    // Prefer appspot bucket first because many Firebase Storage projects still resolve there reliably.
    if (bucketName.endsWith('.firebasestorage.app')) {
      push(bucketName.replace('.firebasestorage.app', '.appspot.com'));
      push(bucketName);
    } else if (bucketName.endsWith('.appspot.com')) {
      push(bucketName);
      push(bucketName.replace('.appspot.com', '.firebasestorage.app'));
    } else {
      push(bucketName);
    }
  }
