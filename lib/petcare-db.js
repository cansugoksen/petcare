...
export async function ensureUserDoc(uid, extra = {}) {
  ...
  await setDoc(
    userDoc(uid),
    {
      ...
      displayName: extra?.displayName ?? existing?.displayName ?? null,
      photoURL: extra?.photoURL ?? existing?.photoURL ?? null,
      isAnonymous: typeof extra?.isAnonymous === 'boolean' ? extra.isAnonymous : !!existing?.isAnonymous,
      providerIds: Array.isArray(extra?.providerIds) ? extra.providerIds : existing?.providerIds || [],
      ...extra,
...
export function subscribeUserDoc(uid, callback, onError) {
  if (!uid) {
    callback?.(null);
    return () => {};
  }
