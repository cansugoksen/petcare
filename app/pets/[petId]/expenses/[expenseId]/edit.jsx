...
import { getExpense, updateExpense } from '@/lib/petcare-db';
...
export default function EditExpenseRoute() {
  ...
  return (
    <AuthGate>
      <EditExpenseScreen petId={petId} expenseId={expenseId} />
    </AuthGate>
  );
}

function EditExpenseScreen({ petId, expenseId }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const item = await getExpense(user.uid, petId, expenseId);
        if (!mounted) return;
        if (!item) {
          setError('Gider kaydı bulunamadı.');
          return;
        }
        setAmountInput(String(item.amount ?? ''));
        setTitle(item.title || '');
        setCategory(item.category || 'vet');
        setExpenseDateInput(toInputDateTime(item.expenseDate || new Date()));
        setClinicName(item.clinicName || '');
        setNote(item.note || '');
        setError('');
      } catch (err) {
        if (mounted) setError(err.message || 'Gider yüklenemedi.');
      } finally {
        if (mounted) setLoading(false);
      }
    }
    if (user?.uid && petId && expenseId) load();
    return () => {
      mounted = false;
    };
  }, [expenseId, petId, user?.uid]);
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      setError('');
      await updateExpense(user.uid, petId, expenseId, {
      ...
      setError(err.message || 'Gider güncellenemedi.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Screen title="Gider Düzenle" subtitle="Kayıt detaylarını güncelleyerek analizleri doğrulayın.">
      <Card style={styles.formCard}>
        <View style={styles.heroStrip}>
          <View style={styles.heroIcon}>
            <MaterialIcons name="edit-note" size={16} color="#2F6E9E" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.heroTitle}>Gider Güncelleme</Text>
            <Text style={styles.heroText}>Tutar, kategori ve tarih değişiklikleri özet kartlarına anında yansır.</Text>
          </View>
        </View>
