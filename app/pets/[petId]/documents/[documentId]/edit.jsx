...
import { getDocument, updateDocument } from '@/lib/petcare-db';
...
export default function EditDocumentRoute() {
  ...
  return (
    <AuthGate>
      <EditDocumentScreen petId={petId} documentId={documentId} />
    </AuthGate>
  );
}

function EditDocumentScreen({ petId, documentId }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    let mounted = true;
    const run = async () => {
      if (!user?.uid || !petId || !documentId) return;
      try {
        setLoading(true);
        const doc = await getDocument(user.uid, petId, documentId);
        if (!mounted) return;
        if (!doc) {
        ...
        setFileType(doc.fileType || 'other');
        setTitle(doc.title || '');
        setDocumentDateInput(toInputDateTime(doc.documentDate || new Date()));
        setClinicName(doc.clinicName || '');
        setNote(doc.note || '');
        setAllergiesText(doc.allergiesText || '');
        setFileUri(doc.fileLocalUri || doc.fileUrl || '');
        setOcrStatus(doc.ocrStatus || 'pending');
      } catch (err) {
        if (mounted) setError(err?.message || 'Belge yüklenemedi.');
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      setError('');
      await updateDocument(user.uid, petId, documentId, {
        title: title.trim(),
        type,
        fileType,
        documentDate,
        ...
        fileName: pickedDocument?.name || pickedImage?.fileName || undefined,
        fileMimeType: pickedDocument?.mimeType || pickedImage?.mimeType || undefined,
        detections: buildInitialDetections({ type, title, note, documentDate, detectedKeywords }),
      });
      router.replace(`/pets/${petId}/documents/index`);
    } catch (err) {
      setError(err?.message || 'Belge güncellenemedi.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <Screen title="Belge Düzenle" subtitle="Yükleniyor..." />;
  }

  return (
    <Screen title="Belge Düzenle" subtitle="Belge bilgilerini güncelleyin.">
      <Card style={styles.formCard}>
        <View style={styles.heroStrip}>
          <View style={styles.heroIcon}>
            <MaterialIcons name="description" size={16} color="#2C6E9E" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.heroTitle}>Belge Detayı</Text>
            <Text style={styles.heroText}>
              OCR durumu: {ocrStatus === 'done' ? 'Hazır' : ocrStatus === 'failed' ? 'Hata' : 'Bekliyor'}
            </Text>
          </View>
        </View>
            ...
            {DOC_TYPES.map((item) => {
              ...
              return (
                <Pressable
                  ...
                  style={({ pressed }) => [styles.typeChip, selected && styles.typeChipSelected, pressed && { opacity: 0.9 }]}>
                  <MaterialIcons name={item.icon} size={14} color={selected ? PetCareTheme.colors.primary : '#607B8D'} />
        ...
        <View style={styles.panel}>
            ...
            {FILE_TYPES.map((item) => {
              ...
              return (
                <Pressable
                  ...
                  style={({ pressed }) => [styles.miniChip, selected && styles.miniChipSelected, pressed && { opacity: 0.9 }]}>
                  <Text style={[styles.miniChipText, selected && styles.miniChipTextSelected]}>{item.label}</Text>
                </Pressable>
              );
            })}
          </View>
          {canPickImage ? <Button title={pickingFile ? 'Seçiliyor...' : 'Fotoğraf Değiştir'} variant="secondary" onPress={pickPhoto} loading={pickingFile} /> : null}
          {canPickDocument ? <Button title={pickingFile ? 'Seçiliyor...' : fileType === 'pdf' ? 'PDF Değiştir' : 'Dosya Değiştir'} variant="secondary" onPress={pickDocument} loading={pickingFile} /> : null}
          ...
          {canPickDocument && pickedDocument ? (
            <View style={styles.docInfoBox}>
              <MaterialIcons name={fileType === 'pdf' ? 'picture-as-pdf' : 'description'} size={18} color="#2C6E9E" />
              <View style={{ flex: 1, gap: 2 }}>
                <Text style={styles.docInfoTitle} numberOfLines={1}>{pickedDocument.name || 'Seçilen belge'}</Text>
                <Text style={styles.docInfoSub} numberOfLines={1}>{pickedDocument.mimeType || 'Dosya'}</Text>
              </View>
            </View>
          ) : null}

          <Field label="Belge URI / Link" value={fileUri} onChangeText={setFileUri} placeholder="Örn. file:///... veya https://..." autoCapitalize="none" />
        </View>

        <View style={styles.panel}>
          <Text style={styles.panelTitle}>Belge Bilgisi</Text>
          <Field label="Başlık" value={title} onChangeText={setTitle} placeholder="Belge başlığı" autoCapitalize="sentences" />
          <Field label="Belge Tarihi" value={documentDateInput} onChangeText={setDocumentDateInput} placeholder="2026-02-23 14:30" />
          <Field label="Klinik / Kurum (opsiyonel)" value={clinicName} onChangeText={setClinicName} placeholder="Klinik adı" autoCapitalize="sentences" />
        </View>

        <View style={styles.panel}>
          <Text style={styles.panelTitle}>Ek Bilgiler</Text>
          <Field label="Alerjiler (opsiyonel)" value={allergiesText} onChangeText={setAllergiesText} placeholder="Örn. Tavuk" autoCapitalize="sentences" />
          <Field label="Not" value={note} onChangeText={setNote} placeholder="Belge notu" multiline autoCapitalize="sentences" />
          <View style={styles.detectBox}>
            <MaterialIcons name="analytics" size={16} color="#6B7F90" />
            <Text style={styles.detectText}>
              Ön tespit: {detectedKeywords.length ? detectedKeywords.join(', ') : 'anahtar kelime bulunmadı'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.footerRow}>
          ...
          <Button title="Güncelle" onPress={handleSave} loading={saving} style={{ flex: 1 }} />
        </View>
      </Card>
    </Screen>
  );
...
const styles = StyleSheet.create({
  ...
  heroTitle: { color: '#2A628D', fontWeight: '700', fontSize: 13 },
  heroText: { marginTop: 1, color: '#6788A2', fontSize: 11, lineHeight: 16 },
  panel: {
    borderWidth: 1,
    borderColor: '#E1EAF2',
  ...
  typeChipSelected: { borderColor: '#BEE0F5', backgroundColor: '#EDF7FF' },
  typeChipText: { color: PetCareTheme.colors.text, fontWeight: '600', fontSize: 12 },
  typeChipTextSelected: { color: PetCareTheme.colors.primary },
  ...
  miniChipSelected: { borderColor: '#BEE0F5', backgroundColor: '#EDF7FF' },
  miniChipText: { color: '#5A778C', fontWeight: '700', fontSize: 12 },
  miniChipTextSelected: { color: PetCareTheme.colors.primary },
  previewWrap: {
    borderWidth: 1,
    borderColor: '#E0EAF2',
    borderRadius: 12,
  ...
  docInfoTitle: { color: PetCareTheme.colors.text, fontWeight: '700', fontSize: 12 },
  docInfoSub: { color: PetCareTheme.colors.textMuted, fontSize: 11 },
  detectBox: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    gap: 8,
  ...
  detectText: { flex: 1, color: '#6A8193', fontSize: 11, lineHeight: 16 },
